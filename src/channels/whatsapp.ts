...
\`lettabot pairing approve whatsapp ${code}\``;
	...
	async start(): Promise<void> {
		...
		try {
			...
			if (procs.trim()) {
				console.warn(
					"[WhatsApp] ⚠️  Warning: clawdbot/moltbot is running and may compete for WhatsApp connection.",
				);
				console.warn(
			...
			for (const m of messages) {
				...
				if (!isGroup && !isSelfChat) {
					...
					if (access === "pairing") {
						...
						const result = await upsertPairingRequest(
							"whatsapp",
							userId,
							pushName,
						);
						...
						if (created) {
							console.log(
								`[WhatsApp] New pairing request from ${userId}: ${code}`,
							);
							await this.sock.sendMessage(remoteJid, {
								text: this.formatPairingMsg(code),
							});
						}
						continue;
					}
