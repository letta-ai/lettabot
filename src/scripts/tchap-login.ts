...
async function main() {
  p.intro('🔑 Tchap Token Generator');

  const homeserverUrl = await p.text({
    message: 'Homeserver URL',
    placeholder: 'https://matrix.dev01.tchap.incubateur.net',
    initialValue: 'https://matrix.dev01.tchap.incubateur.net',
  });
  if (p.isCancel(homeserverUrl)) process.exit(0);

  const username = await p.text({
    message: 'Username (full Matrix ID)',
    placeholder: '@user:dev01.tchap.incubateur.net',
  });
  ...
  try {
    ...
    const client = await auth.passwordLogin(username as string, password as string, "LettaBot");
    ...
    p.log.info(`TCHAP_HOMESERVER_URL=${homeserverUrl}`);
    p.log.info(`TCHAP_ACCESS_TOKEN=${client.accessToken}`);
