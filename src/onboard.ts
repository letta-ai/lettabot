...
const isPlaceholder = (val?: string) =>
	!val || /^(your_|sk-\.\.\.|placeholder|example)/i.test(val);
...
async function stepAuth(
	...
	const { requestDeviceCode, pollForToken, LETTA_CLOUD_API_URL } = await import(
		"./auth/oauth.js"
	);
	const { saveTokens, loadTokens, getOrCreateDeviceId, getDeviceName } =
		await import("./auth/tokens.js");

	const baseUrl =
		config.baseUrl || env.LETTA_BASE_URL || process.env.LETTA_BASE_URL;
	const isLettaCloud =
		!baseUrl ||
	...
	const realApiKey =
		config.apiKey ||
		(isPlaceholder(env.LETTA_API_KEY) ? undefined : env.LETTA_API_KEY);
	const validOAuthToken = isLettaCloud
	...
	const authOptions = [
		...
		{
			value: "selfhosted",
			label: "Enter self-hosted URL",
			hint: "Local Letta server",
		},
		...
		if (existingTokens?.refreshToken) {
			...
			if (isTokenExpired(existingTokens)) {
				...
				try {
					...
					p.log.warning(
						'Your session may have expired. Try "Login to Letta Platform" to re-authenticate.',
					);
				}
			} else {
	...
	if (config.authMethod !== "skip") {
		...
		try {
			...
			if (!ok && config.authMethod === "selfhosted") {
				p.log.warn(
					`Could not connect to ${config.baseUrl}. Make sure the server is running.`,
				);
			}
		} catch {
...
async function stepAgent(
	...
	if (envAgentId) {
		agentOptions.push({
			value: "env",
			label: "Use LETTA_AGENT_ID",
			hint: envAgentId.slice(0, 15) + "...",
		});
	}
...
async function stepProviders(
	...
	config.providers = [];
	const apiKey =
		config.apiKey || env.LETTA_API_KEY || process.env.LETTA_API_KEY;
			...
			try {
				...
				if (existingProvider) {
					...
					response = await fetch(
						`https://api.letta.com/v1/providers/${existingProvider.id}`,
						{
							method: "PATCH",
							headers: {
								"Content-Type": "application/json",
...
async function stepModel(
	...
	const { buildModelOptions, handleModelSelection, getBillingTier } =
		await import("./utils/model-selection.js");
	...
	if (billingTier === "free") {
		p.log.info(
			"Free plan: GLM and MiniMax models are free. Other models require BYOK (Bring Your Own Key).",
		);
	}
...
async function stepChannels(
	...
	const signalInstalled =
		spawnSync("which", ["signal-cli"], { stdio: "pipe" }).status === 0;
		...
		[
			{
				value: "telegram",
				label: "Telegram",
				hint: "Recommended - easiest to set up",
			},
			...
			{ value: "whatsapp", label: "WhatsApp", hint: "QR code pairing" },
			{
				value: "signal",
				label: "Signal",
				hint: signalInstalled
					? "signal-cli daemon"
					: "⚠️ signal-cli not installed",
			},
			{
				value: "matrix",
				label: "Matrix / Tchap",
	...
	while (true) {
		...
		if (channels.length === 0) {
			const skipChannels = await p.confirm({
				message:
					"No channels selected. Continue without any messaging channels?",
				initialValue: false,
			});
			if (p.isCancel(skipChannels)) {
				p.cancel("Setup cancelled");
				process.exit(0);
			}
	...
	if (channels.includes("signal") && !signalInstalled) {
		p.log.warn(
			"Signal selected but signal-cli is not installed. Install with: brew install signal-cli",
		);
	...
	if (config.telegram.enabled) {
			...
			options: [
				{
					...
					hint: "Requires CLI approval",
				},
				{
					value: "allowlist",
					label: "Allowlist only",
					hint: "Specific user IDs",
				},
		...
		if (!p.isCancel(dmPolicy)) {
			...
			if (dmPolicy === "pairing") {
				p.log.info(
					"Users will get a code. Approve with: lettabot pairing approve telegram CODE",
				);
				...
				if (!p.isCancel(users) && users) {
					config.telegram.allowedUsers = users
						.split(",")
						.map((s) => s.trim())
						.filter(Boolean);
				}
	...
	if (config.slack.enabled) {
			...
			options: [
				{
					value: "wizard",
					label: "Guided setup",
					hint: "Step-by-step instructions with validation",
				},
				{
					value: "manual",
					label: "Manual entry",
					hint: "I already have tokens",
				},
			],
		...
		if (wizardChoice === "wizard") {
			...
			const {
				...
				validateAppToken,
				validateBotToken,
			} = await import("./setup/slack-wizard.js");
	...
	if (config.discord.enabled) {
		...
		if (!p.isCancel(dmPolicy)) {
			...
			if (dmPolicy === "pairing") {
				p.log.info(
					"Users will get a code. Approve with: lettabot pairing approve discord CODE",
				);
			} else if (dmPolicy === "allowlist") {
				const users = await p.text({
				...
				if (!p.isCancel(users) && users) {
					config.discord.allowedUsers = users
						.split(",")
						.map((s) => s.trim())
						.filter(Boolean);
				}
	...
	if (config.whatsapp.enabled) {
		...
		const selfChat = await p.select({
			...
			options: [
				{
					value: "dedicated",
					label: "Dedicated bot number",
					hint: "Responds to all incoming messages",
				},
				{
					value: "personal",
					label: "My personal number",
					hint: 'Only responds to "Message Yourself" chat',
				},
			],
		...
		const dmPolicy = await p.select({
			...
			options: [
				...
				},
				{
					value: "allowlist",
					label: "Allowlist only",
					hint: "Specific phone numbers",
				},
				{
					value: "open",
					label: "Open",
					hint: "⚠️ Anyone (not recommended - full account access!)",
				},
			],
		...
		if (!p.isCancel(dmPolicy)) {
			...
			if (dmPolicy === "pairing") {
				p.log.info(
					"Users will get a code. Approve with: lettabot pairing approve whatsapp CODE",
				);
			} else if (dmPolicy === "allowlist") {
				const users = await p.text({
				...
				if (!p.isCancel(users) && users) {
					config.whatsapp.allowedUsers = users
						.split(",")
						.map((s) => s.trim())
						.filter(Boolean);
				}
	...
	if (config.signal.enabled) {
		...
		const selfChat = await p.select({
			...
			options: [
				...
				},
				{
					value: "personal",
					label: "My personal number",
					hint: 'Only responds to "Note to Self" chat',
				},
			],
		...
		if (!p.isCancel(dmPolicy)) {
			...
			if (dmPolicy === "pairing") {
				p.log.info(
					"Users will get a code. Approve with: lettabot pairing approve signal CODE",
				);
			} else if (dmPolicy === "allowlist") {
				const users = await p.text({
					message: "Allowed phone numbers (comma-separated, with +)",
				...
				if (!p.isCancel(users) && users) {
					config.signal.allowedUsers = users
						.split(",")
						.map((s) => s.trim())
						.filter(Boolean);
				}
	...
	if (config.matrix.enabled) {
		p.note(
			"Requires a Matrix account (Tchap or other).\n" +
				"Get access token via Element Web or use npm run matrix-login\n" +
				"See docs/matrix-setup.md for details.",
			"Matrix Setup",
		);
		...
		const homeserver = await p.text({
			...
			initialValue: config.matrix.homeserverUrl || "https://matrix.org",
		});
		if (!p.isCancel(homeserver) && homeserver)
			config.matrix.homeserverUrl = homeserver;

		const token = await p.text({
			message: "Access Token",
			placeholder: "syt_...",
			initialValue: config.matrix.accessToken || "",
		});
		if (!p.isCancel(token) && token) config.matrix.accessToken = token;

		const encryption = await p.confirm({
			message: "Enable End-to-End Encryption?",
			initialValue: config.matrix.encryptionEnabled ?? true,
		});
		if (!p.isCancel(encryption)) config.matrix.encryptionEnabled = encryption;

		// Access control
		const dmPolicy = await p.select({
			message: "Matrix: Who can message the bot?",
			options: [
				{
					value: "pairing",
					label: "Pairing (recommended)",
			...
			initialValue: config.matrix.dmPolicy || "pairing",
		});
		if (!p.isCancel(dmPolicy)) {
			config.matrix.dmPolicy = dmPolicy as "pairing" | "allowlist" | "open";

			if (dmPolicy === "pairing") {
				p.log.info(
					"Users will get a code. Approve with: lettabot pairing approve matrix CODE",
				);
			} else if (dmPolicy === "allowlist") {
				const users = await p.text({
					message: "Allowed Matrix user IDs (comma-separated)",
					placeholder: "@user:matrix.org",
					initialValue: config.matrix.allowedUsers?.join(",") || "",
				});
				if (!p.isCancel(users) && users) {
					config.matrix.allowedUsers = users
						.split(",")
						.map((s) => s.trim())
						.filter(Boolean);
				}
...
async function stepGoogle(config: OnboardConfig): Promise<void> {
	...
	const gogInstalled =
		spawnSync("which", ["gog"], { stdio: "pipe" }).status === 0;
	...
	if (!gogInstalled) {
		...
		const brewInstalled =
			spawnSync("which", ["brew"], { stdio: "pipe" }).status === 0;
		...
		if (brewInstalled) {
			...
			if (installGog) {
				...
				if (result.status === 0) {
					...
					p.log.error(
						"Installation failed. Try manually: brew install steipete/tap/gogcli",
					);
					config.google.enabled = false;
					return;
				}
	...
	const credentialsResult = spawnSync("gog", ["auth", "list"], {
		stdio: "pipe",
	});
	const hasCredentials =
		credentialsResult.status === 0 &&
	...
	if (!hasCredentials) {
		...
		if (!hasCredFile) {
			...
			if (!hasCredentials) {
				p.log.info(
					"Run `gog auth credentials /path/to/client_secret.json` after downloading credentials.",
				);
				config.google.enabled = false;
				return;
			}
	...
	let accounts: string[] = [];
	if (hasCredentials) {
		const listResult = spawnSync("gog", ["auth", "list", "--json"], {
			stdio: "pipe",
		});
		...
		const accountChoice = await p.select({
			...
			options: [
				...
				})),
				{
					...
					hint: "Authorize another account",
				},
			],
...
async function addGoogleAccount(): Promise<string | undefined> {
	...
	const result = spawnSync(
		"gog",
		["auth", "add", email, "--services", (services as string[]).join(",")],
		{
	...
	if (result.status === 0) {
		...
		spinner.stop("Authorization failed");
		p.log.error(
			"Failed to authorize account. Try manually: gog auth add " + email,
		);
		return undefined;
	}
}
...
function showSummary(config: OnboardConfig): void {
	...
	const authLabel = {
		...
		apikey: config.apiKey
			...
			: "API Key",
		selfhosted: config.baseUrl
			? `Self-hosted (${config.baseUrl})`
			: "Self-hosted",
		skip: "None",
	}[config.authMethod];
	...
	if (config.whatsapp.enabled)
		channels.push(config.whatsapp.selfChat ? "WhatsApp (self)" : "WhatsApp");
	if (config.signal.enabled)
		channels.push(config.signal.selfChat ? "Signal (self)" : "Signal");
	if (config.matrix.enabled)
		channels.push(config.matrix.encryptionEnabled ? "Matrix (E2EE)" : "Matrix");
	lines.push(
	...
	if (config.heartbeat.enabled)
		features.push(`Heartbeat (${config.heartbeat.interval}m)`);
	...
	if (config.google.enabled) {
		lines.push(
			`Google:    ${config.google.account} (${config.google.services?.join(", ") || "all"})`,
		);
	}
...
export async function onboard(): Promise<void> {
	...
	const config: OnboardConfig = {
		...
		matrix: {
			...
			encryptionEnabled: existingConfig.channels.matrix?.encryptionEnabled,
		...
		providers: existingConfig.providers?.map((p) => ({
			id: p.id,
			name: p.name,
			apiKey: p.apiKey,
		})),
	...
	if (config.authMethod !== "selfhosted" && config.agentChoice === "new") {
		...
		config.billingTier = billingTier ?? undefined;
		spinner.stop(
			billingTier === "free" ? "Free plan" : `Plan: ${billingTier || "Pro"}`,
		);
	}
	...
	if (config.telegram.enabled && config.telegram.token) {
		...
		if (config.telegram.dmPolicy)
			env.TELEGRAM_DM_POLICY = config.telegram.dmPolicy;
	...
	if (config.discord.enabled && config.discord.token) {
		...
		if (config.discord.dmPolicy)
			env.DISCORD_DM_POLICY = config.discord.dmPolicy;
	...
	if (config.whatsapp.enabled) {
		...
		if (config.whatsapp.dmPolicy)
			env.WHATSAPP_DM_POLICY = config.whatsapp.dmPolicy;
	...
	if (
		...
		env.MATRIX_HOMESERVER_URL = config.matrix.homeserverUrl;
		env.MATRIX_ACCESS_TOKEN = config.matrix.accessToken;
		if (config.matrix.encryptionEnabled === false)
			env.MATRIX_ENCRYPTION_ENABLED = "false";
		else delete env.MATRIX_ENCRYPTION_ENABLED; // default true

		if (config.matrix.dmPolicy) env.MATRIX_DM_POLICY = config.matrix.dmPolicy;
		if (config.matrix.allowedUsers?.length) {
			env.MATRIX_ALLOWED_USERS = config.matrix.allowedUsers.join(",");
		} else {
			delete env.MATRIX_ALLOWED_USERS;
		}
	} else {
		delete env.MATRIX_HOMESERVER_URL;
		delete env.MATRIX_ACCESS_TOKEN;
		delete env.MATRIX_ENCRYPTION_ENABLED;
	...
	const formatAccess = (policy?: string, allowedUsers?: string[]) => {
		...
		if (policy === "allowlist")
			return `allowlist (${allowedUsers?.length || 0} users)`;
	...
	const summary = [
		...
		config.telegram.enabled
			? `  ✓ Telegram (${formatAccess(config.telegram.dmPolicy, config.telegram.allowedUsers)})`
			: "  ✗ Telegram",
		config.slack.enabled
			? `  ✓ Slack ${config.slack.allowedUsers?.length ? `(${config.slack.allowedUsers.length} allowed users)` : "(workspace access)"}`
			: "  ✗ Slack",
		config.discord.enabled
			? `  ✓ Discord (${formatAccess(config.discord.dmPolicy, config.discord.allowedUsers)})`
			: "  ✗ Discord",
		config.whatsapp.enabled
			? `  ✓ WhatsApp (${formatAccess(config.whatsapp.dmPolicy, config.whatsapp.allowedUsers)})`
			: "  ✗ WhatsApp",
		config.signal.enabled
			? `  ✓ Signal (${formatAccess(config.signal.dmPolicy, config.signal.allowedUsers)})`
			: "  ✗ Signal",
		config.matrix.enabled
			? `  ✓ Matrix (${formatAccess(config.matrix.dmPolicy, config.matrix.allowedUsers)})`
			: "  ✗ Matrix",
		"",
		"Integrations:",
	...
	const yamlConfig: LettaBotConfig = {
		server: {
			...
			...(config.authMethod === "selfhosted" && config.baseUrl
		...
		features: {
			...
			heartbeat: {
				...
				intervalMin: config.heartbeat.interval
					? parseInt(config.heartbeat.interval)
					: undefined,
			},
		},
		...(config.google.enabled
	...
	if (
		...
		yamlConfig.providers.length > 0 &&
		yamlConfig.server.mode === "cloud"
	) {
		const spinner = p.spinner();
